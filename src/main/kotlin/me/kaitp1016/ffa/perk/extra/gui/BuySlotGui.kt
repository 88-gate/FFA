...
class BuySlotGui: ChestPacketGui {
    ...
    val playerPerkData: ExtraPerkManager.PlayerExtraPerkData

    constructor(player: ServerPlayer, parent: AbstractPacketGui?) : super(player, 27) {
        this.parent = parent
        this.playerPerkData = ExtraPerkManager.getPerkData(player.uuid)
        ...
        setItem(PREVIEW_SLOT, ItemStack(Items.LIME_STAINED_GLASS_PANE).apply {
            ...
            set(DataComponents.LORE, ItemLore(listOf(Component.literal("必要ポイント: ${ExtraPerkManager.getUnlockCost(playerPerkData.unlockedSlots + 1)}").withStyle(Style.EMPTY.withItalic(false).withColor(0xFFFFFF)))))
        })
    ...
    override fun onClick(packet: ServerboundContainerClickPacket) {
        mc.execute {
            ...
            when (slot) {
                BUY_SLOT -> {
                    val cost = ExtraPerkManager.getUnlockCost(playerPerkData.unlockedSlots + 1)
                    ...
                    playerPerkData.unlockedSlots++
                    player.addMoney(-cost)
                    playerPerkData.selectedPerks.add(ExtraPerks.EMPTY)
