...
class ExtraPerkMainGui: ChestPacketGui {
    ...
    override fun onClick(packet: ServerboundContainerClickPacket) {
        ...
        if (!isUnlocked(index)) {
            val cost = ExtraPerkManager.getUnlockCost(perk.unlockedSlots + 1)
    ...
    private fun getPerkItem(slot: Int): ItemStack {
        val index = getPerkIndex(slot)
        if (!isUnlocked(index)) {
            val cost = ExtraPerkManager.getUnlockCost(this@ExtraPerkMainGui.perk.unlockedSlots + 1)
            return ItemStack(Items.RED_STAINED_GLASS_PANE).apply {
                set(DataComponents.ITEM_NAME, Component.literal("Extra Perk #${index + 1} (未開放)").withColor(0xFF55FF))
        ...
        val perk = this@ExtraPerkMainGui.perk.selectedPerks.getOrNull(index) ?: ExtraPerks.EMPTY

        return perk.icon.copy().apply {
            set(DataComponents.ITEM_NAME, Component.literal("Extra Perk #${index + 1}").withColor(0xFF55FF).append(Component.literal(" (${perk.name})").withColor(0xFFFF55)))
