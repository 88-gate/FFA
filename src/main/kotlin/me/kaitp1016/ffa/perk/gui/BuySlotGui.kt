...
class BuySlotGui: ChestPacketGui {
    ...
    constructor(player: ServerPlayer, parent: AbstractPacketGui?) : super(player, 27) {
        ...
        setItem(PREVIEW_SLOT, ItemStack(Items.LIME_STAINED_GLASS_PANE).apply {
            ...
            set(DataComponents.LORE, ItemLore(listOf(Component.literal("必要ポイント: ${PerkManager.getUnlockCost(playerPerkData.unlockedSlots + 1)}").withStyle(Style.EMPTY.withItalic(false).withColor(0xFFFFFF)))))
        })
    ...
    override fun onClick(packet: ServerboundContainerClickPacket) {
        mc.execute {
            ...
            when (slot) {
                BUY_SLOT -> {
                    val cost = PerkManager.getUnlockCost(playerPerkData.unlockedSlots + 1)
                    ...
                    playerPerkData.selectedPerks.add(Perks.EMPTY)
