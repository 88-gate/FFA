...
class PerkSelectGui: ChestPacketGui {
    ...
    var index: Int
    val perkData: PerkManager.PlayerPerkData
    val perks:List<Perk>

    constructor(player: ServerPlayer, index: Int, parent: AbstractPacketGui?) : super(player, 36) {
        this.parent = parent
        this.index = index
        this.perkData = PerkManager.getPerkData(player.uuid)
        this.perks = Perks.entries()
    }
    ...
    override fun onClick(packet: ServerboundContainerClickPacket) {
        ...
        mc.execute {
            val count = perkData.selectedPerks.count { it == perk }

            if (count >= perk.duplicateLimit) {
                player.sendSystemMessage(Component.literal("§cこのパークは${perk.duplicateLimit}個までしか重複できません!"))
                player.connection.send(ClientboundSoundPacket(Holder.direct(SoundEvents.SPLASH_POTION_BREAK), SoundSource.MASTER, player.x, player.y, player.z, 1f, 1f, 1L))
                update(false)
                return@execute
            }
            ...
            player.connection.send(ClientboundSoundPacket(Holder.direct(SoundEvents.EXPERIENCE_ORB_PICKUP), SoundSource.MASTER, player.x, player.y, player.z, 1f, 1f, 1L))
            openParent()
            PerkAttributeHandler.update(player.bukkitEntity)
        }
    }

    private fun getPerkItem(perk: Perk): ItemStack {
        if (!isUnlocked(perk)) {
    ...
    private fun isUnlocked(perk: Perk): Boolean {
